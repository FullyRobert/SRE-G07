                        	...
                        	<form role="form" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" class="registration-form">
			                    ...
			                    <fieldset>
		                            ...
		                            <div class="form-bottom">
                                        <?php
                                        if(isset($_POST['signup_submit'])){//判断用户是否提交注册表单，如果是则执行如下代码
                                            $username = $_POST['form-name'];
                                            $id = $_POST['form-id'];
                                            $usertype = $_POST['form-usertype'];
                                            $password = $_POST['form-password'];
                                            $mail = $_POST['form-email'];
                                            $question = $_POST['form-question'];
                                            $answer = $_POST['form-answer'];
                                            $query = "SELECT * FROM user_signup WHERE name = '$username'";
                                            ...
                                            if($result){
                                                if(mysqli_num_rows($result)>0){
                                                    ...
                                                    $query = "insert into user_signup(id,password, name, mail,question, answer,usertype) values ('$id','$password','$username','$mail','$question','$answer','$usertype')";
                                                    $result = @mysqli_query($dbc, $query);
                                                    if($result) {
                                                        if ($usertype == '2'){
                                                            ...
                                                            $query = "insert into teacher(tid, password, tname) values ($id,'$password','$username')";
                                                        }
                                                        else if ($usertype == '3'){
                                                            $query = "insert into TA(aid, password, aname) values ($id,'$password','$username')";
                                                        }
                                                        $result = @mysqli_query($dbc, $query);
                                                        if($result){
																...
																if ($usertype == '2')
																	echo "<script>alert('注册成功，请重新登陆');location.href='./student/index.php';</script>";
																else if ($usertype == '1')
																	echo "<script>alert('注册成功，请重新登陆');location.href='./teacher/index.php';</script>";
																else if ($usertype == '3')
																	echo "<script>alert('注册成功，请重新登陆');location.href='./ta/index.php';</script>";
                                        ...
                                        <div class="form-group">
				                        	<label class="sr-only" for="form-email">学工号</label>
				                        	<input type="text" name="form-id" placeholder="学工号..." class="form-id form-control" id="form-id">
				                        </div>
				                        <div class="form-group">
										...
										<div class="form-group">
										<select class="form-control" name="form-usertype">
												  <option value ="选择用户类型" selected class="form-control">选择用户类型</option>
