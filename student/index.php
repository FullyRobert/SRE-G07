                        ...
                        <div class="row">
                            <div class="col-md-8 col-md-offset-2 text-center slider-text">
                                <div class="slider-text-inner">
                                    <h1>面向全日制高校的教学平台</h1>
                                    <h2>辅助教学系统</a></h2>
                                    <p><a class="btn btn-primary btn-lg" id="login" data-toggle="modal" data-target="#myModal1" href="#">加入课程</a></p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-8 col-md-offset-2 text-center slider-text">
                                <div class="slider-text-inner">
                                    <h1>全日制高校 / 教学资源</h1>
                                    <h2>帮助更多师生</h2>
                                    <p><a class="btn btn-primary btn-lg btn-learn" id="login" data-toggle="modal" data-target="#myModal1" href="#">加入课程</a></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-content">
                    <form class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
                        <div class="modal-header btn-info">
                            <span class="heading modal-title" id="myModalLabe2">加入课程</span>
                        </div>
                        <div class="modal-body">
							<?php
                                if(isset($_POST['submit'])){//判断用户是否提交注册表单，如果是则执行如下代码
									...
									$sid = $_COOKIE['user_id'];
                                    $query = "select * from course_basic where course_id = $course_id";
                                    $result = @mysqli_query($dbc, $query);
									if($result){
										if(mysqli_num_rows($result)>0){
											$query = "insert into course(course_id, sid) values ('$course_id', '$sid')";
											$result = @mysqli_query($dbc, $query);
											if($result){
											...
											echo "<script>alert('验证码无效，加入失败！');location.href='index.php';</script>";
										}
									}
								}											
                            ?>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <?php
                            require_once('../php/mysqli_connect.php');
                            $sid = $_COOKIE['user_id'];
                            $notice_query = "select course_id, user_type, content, time from notice where course_id in (select course_id from course where sid = '$sid') or user_type = 0 order by id desc;";
                            $notice_result = @mysqli_query($dbc, $notice_query);
                            $notice_index = 0;
                            $notice_num = 10;
                            while ($notice_tmp_row = mysqli_fetch_array($notice_result, MYSQLI_ASSOC)) {
                                $notice_index += 1;
                                if ($notice_index > $notice_num) break;
        ...
        <div class="container">
            <div class="row row-pb-md">
                ...
                <div class="col-md-5 col-sm-4 col-xs-6 col-md-push-1 fh5co-widget">
                    <h3>网站向导</h3>
                    <ul class="fh5co-footer-links">
							<li><a href="">此网站面向全日制高校的课程教学。<br >
							游客可以进入网站查看文章，其他功能暂未开放。<br />
							学生登录后，查看除文章以外的作业、资料和论坛模块。<br />
							教师登录后，发布和点评作业、上传课程资料、进入小组论坛进行指导、发布文章和对公告进行管理。</a></li>
					</ul>
                </div>
            </div>
        </div>
